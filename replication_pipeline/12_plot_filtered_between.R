...
final_plot <- ggplot(combined_results, aes(x = temperature, y = ka_mean,
                       colour   = type,
                       linetype = type)) +
  geom_line(size = 1) +
  ...
  scale_colour_manual(
    name   = "Type",
    breaks = c("Original", "Filtered", "Filtered & Balanced"),
    values = c(
      Original              = "black",
      Filtered              = "#E69F00",
  ...
  # collapse colour + linetype into one legend
  guides(
    colour   = guide_legend(order = 1, override.aes = list(size = 1.2)),
